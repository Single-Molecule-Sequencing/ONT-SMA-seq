...
The overhaul turns the original NextFlow pipeline into an efficient database structure using distinct Python scripts to separate concerns: database creation, input standardization (parsing filename metadata), core ingestion logic, and accessory outputs.

* **Language:** Python 3.x
* **Key Libraries:** `pysam` (BAM I/O), `pod5` (End Reason extraction), `edlib` (Levenshtein), `sqlite3`, `argparse`.
...
* **Purpose:** Standardizes input file paths and extracts metadata from the incoming BAM filename.
* **Inputs (via argparse):**
...
* **Logic:**
    1.  **Parse BAM Filename:** Extract metadata components (`exp_id`, `model`, `ver`, `trim`, `mods`) directly from the input BAM filename using string splitting/regex.
    2.  Create `Input/` directory if it doesn't exist.
    3.  **Symlink BAM:** Source $\to$ `Input/{exp_id}_{bc_model_type}_v{bc_model_version}_{trim}_{modifications}.bam` (Standardizing location).
    4.  **Symlink Pod5:** Source Dir $\to$ `Input/{exp_id}_pod5/`.
    5.  **Symlink RefSeq:** Source FASTA $\to$ `Input/{exp_id}.fa`.
    6.  **Persist Metadata:** (Optional) Save the parsed metadata (like `trim` status or `model_ver`) to a JSON or temp file, or pass them to the next script if run in a wrapper. Ideally, `ingest.py` can re-parse the filename from the now-standardized `Input/` folder to get these values.

#### **Script C: `ingest.py` (Main Processing)**
...
* **Logic Flow:**
    1.  **Parse Reference (FASTA):**
        ...
        * Create Dictionary: `{defline: [seq, len, range]}`.
        * **DB Action:** Insert entries into the `Refseq` table.
    2.  **Parse Pod5 Metadata:**
        * Iterate through all files in `Input/{exp_id}_pod5/`.
        * Build a lookup dictionary: `{read_id: end_reason_string}`.
    3.  **Stream & Process Reads:**
        ...
        * **Loop through every read:**
            1.  **ER Lookup:** Retrieve End Reason from Pod5 dict.
